...
export default function ContactForm() {
  ...
  async function onSubmit(data: IFormData) {
    ...
    try {
      const response = await fetch("https://formsubmit.co/ajax/carlosmatateumateus@gmail.com", {
        method: "POST",
        headers: {
    ...
    >
      ...
      <div className="flex flex-col">
        ...
        <input
          ...
          {...register("subject")}
        />
        <p className="text-red-500 font-medium mt-1">{errors.subject?.message}</p>
      </div>
      <div className="flex flex-col">
        <label>Message</label>
        <textarea
          ...
          {...register("message")}
        />
        <p className="text-red-500 font-medium mt-1">{errors.message?.message}</p>
      </div>
